...
<div class="container">
    ...
    <form action="{{ route('import.workflow') }}" id="importWorkFlow" method="POST" enctype="multipart/form-data">
        @csrf
        <x-drop-area accept=".xls,.xlsx" />
        <button type="submit" class="btn btn-primary"> <i class="bi bi-upload me-1"></i> Import</button>
        <a href="/sample_sheets/workflow_sample.xlsx" target="_blank" download class="ms-1 text-decoration-none">Download Sample File</a>
    </form>
</div>
@endsection
